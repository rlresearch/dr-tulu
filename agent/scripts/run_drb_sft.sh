...
MODEL_PORT=30001
MCP_PORT=8000
OUTPUT="eval_output/dr_tulu_sft_8b/deep_research_bench.jsonl"
...
for i in $(seq 1 180); do
    ...
    if ! kill -0 $VLLM_PID 2>/dev/null; then
        echo "ERROR: vLLM died"; tail -50 /tmp/vllm_server.log; exit 1
    fi
    [ $i -eq 180 ] && { echo "TIMEOUT"; tail -50 /tmp/vllm_server.log; exit 1; }
    sleep 5
done
...
for i in $(seq 1 60); do
    ...
    if ! kill -0 $MCP_PID 2>/dev/null; then
        echo "ERROR: MCP died"; tail -50 /tmp/mcp_server.log; exit 1
    fi
    [ $i -eq 60 ] && { echo "TIMEOUT"; tail -50 /tmp/mcp_server.log; exit 1; }
    sleep 5
done

echo "=== Running DRB generation ==="
python workflows/auto_search_sft.py \
    generate-dataset deep_research_bench \
